...
while (TRUE){
  coin <- round( runif(1) )  # redondear al entero m谩s cercano
  if (coin == 1) {
    break
  } else {
    w = w + 10
...
tryCatch(a/7,
         ...
         error = function(e)  {
           ...
           cat("Correci贸n del tipo de variable:" , as.integer(a) / 7)
...
calculator_square <- function(x,y){
  ...
  return(list(x_2 = x2,
              y_2 = y2,
              text = paste0("La multiplicaci贸n del cuadrado es:", result)) )
}
