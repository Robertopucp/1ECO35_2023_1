...
spend_years %>% ggplot( aes(x = año, y = gasmpc, group = area, colour = area) ) +
  ...
  theme_bw() + # diseño de fondo
  scale_x_continuous(breaks = c(2015, 2016, 2017, 2018, 2019 , 2020) ) +
  ggtitle("Average monthly spenditure percapita by area")+
  xlab("")+
  ylab("")
...
# no fijamos en los nombres de las variables de cada módulo
names(rech84dv)
names(rech0)
names(rech1)
names(rech23)
...
# no tenemos la columna HHID en rech84dv pero vemos que se parece a la columna caseid pero con menos caracteres
rech84dv$HHID <- substring(rech84dv$caseid, 1, 15)

#ahora si lo unimos con las demas bases de datos
